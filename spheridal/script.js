const setWhite = (cx, alpha=1) => {
    // cx.fillStyle = `rgba(255, 255, 255, ${alpha})`;
    // cx.strokeStyle = `rgba(255, 255, 255, ${alpha})`;
    cx.fillStyle = `rgba(225, 225, 225, ${alpha})`;
    cx.strokeStyle = `rgba(255, 255, 255, ${alpha})`;
}

const setBlack = (cx, alpha=1) => {
    cx.fillStyle = `rgba(25, 25, 25, ${alpha})`;
    cx.strokeStyle = `rgba(0, 0, 0, ${alpha})`;
}

const euclideanDistance = (center1, center2) => {
...
const drawYinYanEccentric = (center1={'x': 511, 'y': 441}, center2={'x': 923, 'y': 741}) => {
    ...
    const radius = euclideanDistance(center1, center2);
    ...
    setWhite(cx);
    cx.beginPath();
    cx.arc(centerCenter.x, centerCenter.y, radius, 0, 2 * Math.PI, false);
    cx.fill();cx.stroke();

    // draw black large semicircle
    setBlack(cx);
    cx.beginPath();
    cx.arc(centerCenter.x, centerCenter.y, radius, end, start, false);
    cx.fill();cx.stroke();

    // draw white medium circle
    setWhite(cx);
    cx.beginPath();
    cx.arc(center1.x, center1.y, radius / 2, 0, 2 * Math.PI, false);
    cx.fill();cx.stroke();

    // draw black medium circle
    setBlack(cx);
    cx.beginPath();
    cx.arc(center2.x, center2.y, radius / 2, 0, 2 * Math.PI, false);
    cx.fill();cx.stroke();

    // draw white small circle
    setWhite(cx);
    cx.beginPath();
    cx.arc(center2.x, center2.y, radius / (2 * Math.PI), 0, 2 * Math.PI, false);
    cx.fill();cx.stroke();

    // draw black small circle
    setBlack(cx);
    cx.beginPath();
    cx.arc(center1.x, center1.y, radius / (2 * Math.PI), 0, 2 * Math.PI, false);
    cx.fill();cx.stroke();
        
}


const drawYinYanEccentric1 = (center1={'x': 500, 'y': 440}, center2={'x': 800, 'y': 640}) => {
    
    const centerCenter = {};
    
    centerCenter.x = (center1.x + center2.x) / 2;
    centerCenter.y = (center1.y + center2.y) / 2;
    
    const radius = Math.sqrt(Math.pow(center1.x - center2.x, 2) + Math.pow(center1.y - center2.y, 2));
    ...
    cx.strokeStyle = 'rgba(255, 255, 255, 1.0)';

    //cx.strokeStyle = 'white';

    cx.beginPath();
    cx.arc(center1.x, center1.y, radius, 1.2 * Math.PI / 6, (8/6) * Math.PI, false);
    cx.strokeStyle = "rgba(100, 100, 100, 0.5)";
    cx.lineWidth = "1";
    //cx.globalCompositeOperation = 'difference'; //cx.filter = 'invert(0)';
    cx.stroke();
    ...
    cx.beginPath();
    cx.arc(centerCenter.x, centerCenter.y, radius, Math.PI, Math.PI, false);
    //cx.strokeStyle = "rgba(100, 100, 100, 1)";
    cx.lineWidth = "1";
    //cx.globalCompositeOperation = 'difference'; //cx.filter = 'invert(0)';
    cx.stroke();
    ...
    start = Math.atan2(center1.y - center2.y, center1.x - center2.x);
    end = start + Math.PI;
    cx.beginPath();
    cx.arc(center1.x, center1.y, radius / 2, start, end, false);
    //cx.strokeStyle = "rgba(100, 100, 100, 1)";
    cx.lineWidth = "1";
    //cx.globalCompositeOperation = 'difference'; //cx.filter = 'invert(0)';
    cx.stroke();
    ...
    cx.beginPath();
    cx.arc(center2.x, center2.y, radius / 2, start, end, false);
    //cx.strokeStyle = "rgba(100, 100, 100, 1)";
    cx.lineWidth = "1";
    //cx.globalCompositeOperation = 'difference'; //cx.filter = 'invert(1)';
    cx.stroke();
    cx.fill();
...
const drawPlanet = (planet={'radius': 50, 'mass': 4}, center={'x': 600, 'y': 440}) => {
    
    cx.beginPath();
    cx.arc(center.x, center.y, planet.radius * 10, 0, 2 * Math.PI, false);
    cx.strokeStyle = "rgba(100, 100, 100, 1)";
    cx.lineWidth = `${planet.mass / 2}`;
    cx.stroke();
}
...
const drawCircle = (center={'x': 600, 'y': 440}, radius) => {
...
const fillCircle = (center={'x': 600, 'y': 440}, radius) => {
    cx.globalCompositeOperation = 'difference';
    //cx.filter = 'invert(0)';
    cx.beginPath();
    cx.arc(center.x, center.y, radius, 0, 2 * Math.PI, false);
    ...
    // cnvs.style.width = 600;
    // cnvs.style.height = 600;
    ...
    // cnvs.setAttribute('width', 0.5 * parseInt(window.getComputedStyle(cnvs, null).getPropertyValue('width')));
    // cnvs.setAttribute('height', 0.5 * parseInt(window.getComputedStyle(cnvs, null).getPropertyValue('height')));
    // cnvs.setAttribute('width', 600);
    // cnvs.setAttribute('height', 600);


    // get context
    let cx = cnvs.getContext("2d");

    const sunCenter = {'x': cnvs.width / 2, 'y': cnvs.height / 1.01};
    //const sunCenter = {'x': cnvs.width / 2, 'y': cnvs.height / 2};

    let increment = 1;
    let minRadPow = 0;
    for (let i = 0; i < Math.log2(cnvs.height * cnvs.height); i += increment) {

// larger numbers for max i introduce a 45 degree artifact that duplicates as max i grows larger--why?
// happens between
    // for (let i = 0; i < 1.1 * Math.log2(cnvs.height * cnvs.height); i += increment) {
//       AND
    // for (let i = 0; i < 1.11 * Math.log2(cnvs.height * cnvs.height); i += increment) {


    //for (let i = 0; i < Math.ceil(Math.log2(cnvs.height * cnvs.height * cnvs.height)); i += increment) {
    //for (let i = 0; i < 100/*Math.log2(cnvs.height * cnvs.height)*/; i += increment) {
    //for (let i = minRadPow; i < 10; i += increment) {

        // if (i / Math.pow(i, 0.5) % 1 === 0 ) continue; // interesting branch to explore
        

        let radius = (Math.pow(2, i));
        //drawCircle({'x': sunCenter.x, 'y': sunCenter.y - radius }, radius); 

        console.log(i);
        for (let j = i; j > minRadPow; j--)
        {
            // let radius2 = (Math.pow(2, j));
            // drawCircle({'x': sunCenter.x, 'y': sunCenter.y - radius }, radius2);
            // drawCircle({'x': sunCenter.x, 'y': sunCenter.y - radius }, radius / j);

            
            //drawCircle({'x': sunCenter.x, 'y': sunCenter.y - radius }, radius / Math.pow(2, i - j));
            fillCircle({'x': sunCenter.x, 'y': sunCenter.y - radius }, radius / Math.pow(2, i - j));
        }
