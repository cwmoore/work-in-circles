...
const drawCircle = (planet={'radius': 50, 'mass': 4}, center={'x': 600, 'y': 440}) => {
...
const mercDists = [// mercury distances to sun
    0.5,
    10,
    19,
    26,
...
const sqrtDists = [//sqrt distance to sun
    1,
    7.61,
    10.40,
    12.23,
...
// drawYinYanEccentric();
//sqrtDists.map((rad) => drawYinYanEccentric({'x':sunCenter.x, 'y': sunCenter.y}, {'x': sunCenter.x - rad, 'y': sunCenter.y + rad}));
...
//         setInterval(() => drawYinYanEccentric(sunCenter, {'x': Math.pow(2, i / j), 'y': counter++ % Math.pow(2, j / i)})
//             , 0.1);
//     }
// }
...
for (let day = 0; day < yearDays[6]; day += increment) {
    ...
    cx.fillRect(0, 0, cx.width, cx.height);
            ...
            sqrtDists.map((radius, index) => {
                // let ret = setInterval(
                //     () => { 
                        if (index > 5) return;
                        radius *= 12;//0.99;//0.618;

                        let rotation = day / yearDays[index];

                        let radians = rotation * 2 * Math.PI;
                        let radX = radius * Math.cos(radians);
                        let radY = -radius * Math.sin(radians);
                        
                        drawYinYanEccentric(sunCenter, {'x': sunCenter.x + radX, 'y': sunCenter.y + radY}) 
            }
                //     , 20
                // );
            //}
            );
        //if (day > 3000) window.location = cnvs.toDataURL();
        
        // if (day % 3000 == 0) {
        //     window.open(cnvs.toDataURL(), '_blank');
        // }
