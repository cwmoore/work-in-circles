...
const makeFillStyles = (numFills = 10) => {
    ...
    // const fillStyles = colors.map( (colorR) => {
    //     return colors.map( (colorG) => {
    //         return colors.map( (colorB) => {
    //             return `rgba(${colorR}, ${colorG}, ${colorB}, 0.15)`;
    //         })[4];
    //     })[5];
    // });

    //console.table(fillStyles);

    //return fillStyles;
    //return ['red', 'blue', 'green', 'white', 'yellow', 'lilac'];
    return [
        'rgba(99, 199, 19, 1.0)',
        'rgba(99, 199, 60, 1.0)',
        'rgba(99, 199, 160, 1.0)',
        ...
        'rgba(199, 199, 19, 1.0)',
        'rgba(199, 199, 60, 1.0)',
        'rgba(199, 199, 160, 1.0)',
        ];
}

const drawPieSlice = (center, radius, portion, priorPortion = 0, fillStyle='rgba(99, 199, 19, 1.0)') => {

    let zeroPos = 0 + Math.PI / 4;

    let angleStart = zeroPos + (priorPortion * 2 * Math.PI);
    let angleEnd = zeroPos + ((priorPortion + portion) * 2 * Math.PI);
    
    cx.beginPath();
    cx.moveTo(center.x, center.y);

    cx.arc(center.x, center.y, radius, angleStart, angleEnd, false);
...
const circoOdd = () => {
    ...
    //let radiusIncrement = 2.5 / Math.PI;
    let radiusIncrement = 1.25;
    
    let maxCounter = 300;
    let radius = maxCounter * radiusIncrement;

    let fillStyles = makeFillStyles();

    for (let i = maxCounter; i > 0; i--)
    {
        cx.beginPath();

        let areaInUnitCircles = 2 * i + 1;
        for (let j = 1; j <= areaInUnitCircles; j++)
        {
            // console.log(i, j);
            drawPieSlice(center, radius, 1 / (areaInUnitCircles), j / areaInUnitCircles, fillStyles[j % fillStyles.length]);
            //return;
        }
        
        radius -= radiusIncrement;
        //if (i + 3 - maxCounter == 0) return;
    }
}

const circoOdd10ToBlack = () => {
    let radiusIncrement = 100 / Math.PI;
    let radius = radiusIncrement;
    ...
    for (let i = 1; i < 10; i++)
    {
        ...
        cx.fillStyle = "rgba(9, 99, 159, " + (1 / i).toFixed(2) + ")";
        cx.fill();
